...
console.log('%d %o', i, files[i]);
		...
		flow.on('fileProgress', function(file, chunk){
			...
			updateProgress(el, p, 'uploading');
    		console.log('progress %o %o. %o pending uploads', file._prevUploadedSize, file, pendingUpload);
		});
		flow.on('fileSuccess', function(file,message){
			var el = findElement(file.name, file.size);
			updateProgress(el, -1, 'completed', 'rgba(200,250,180,0.3)');
            pendingUpload--;
            console.log('success %o %o. %o files left', file, message, pendingUpload);
            if (pendingUpload == 0) completionCallback();
		});
		flow.on('fileError', function(file, message){
            pendingUpload--;
            console.log('error %o %o. %o files left', file, message, pendingUpload);
            if (pendingUpload == 0) completionCallback();
		});
